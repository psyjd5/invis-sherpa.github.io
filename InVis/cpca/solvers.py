...
def async_rom_eig_vals(eig_sec_solver, args):
    try:
        ...
        warnings.warn('error while computing the eigenvalue using secular eigen-solver...')
        return None
...
def async_rom_eig_vec(i, delta_vec, v, diag):
    try:
        ...
        warnings.warn('error while computing the eigenvector using secular-eigen solver...')
        return None
'''
    ###############################                        ###############################
'''
        ...
        if self.__is_outer_root(i, d.shape[0]):
            ...
            while k < self.max_iters:
                ...
                if eta < self.precision:
                    ...
                    warnings.warn('gander secular solver has derivative 0 at the candidate point!')
                    break
            ...
            if k == self.max_iters:
                ...
                warnings.warn('max number of allowed iterations reached with the gander secular solver (value at root = ' + str(at_root) + ')')
...
class eigen_secular_solver(secular_solver):
    ...
    def _inner_init_root(self, v, sigmas, split_idx, axis_shift):
        ...
        c = self.secular_eval(midpoint, v[:(split_idx - 1)], sigmas[:(split_idx - 1)]) + self._phi_eval(midpoint, v[(split_idx + 1):], sigmas[(split_idx + 1):])
        delta0 = sigmas[split_idx] - sigmas[split_idx - 1]
        if np.abs(axis_shift - sigmas[split_idx]) < self.precision:
            a = -c * delta0 + v[split_idx - 1] * v[split_idx - 1] + v[split_idx] * v[split_idx]
...
#                warnings.warn(str(axis_shift) + ' - closing to the right end point of the interval (' + str(sigmas[split_idx - 1]) + ', ' +  str(sigmas[split_idx]) + ')! Delta = ' + str(rep_delta))    
    ...
    def _outer_root(self, v, sigmas):
        ...
        if itr == self.max_iters:
            warnings.warn('max number of allowed iterations reached with the eigen secular solver (value at _outer_ root = ' + str(sec_at_root) + ')')
...
class givens_transformer(object):
    ...
    def transform(self, A, i, j, givens_coeff):
        rotation = np.array([[givens_coeff[0], -givens_coeff[1]], [givens_coeff[1], givens_coeff[0]]])  
...
class appended_column_qr_solver(object):
    ...
    def qr_transform(self, W, Q, R, alpha):
        n = Q.shape[0]
        m = 1 + R.shape[1] if None != R else 1
        R_ = np.zeros((n, m))
        R_[:, :(m - 1)] = R
        Q = Q.T
...
#        self._eig_solver = graggs_eigen_secular_solver(precision, max_iters)
    ...
    def _eig_vecs(self, delta_vecs, v, diag):
        ...
        for i in range(n):
            t = diag[i] - diag
            t[i] = -1.0
            eig_vecs[i, :] = np.sign(v[i]) * np.sqrt(np.abs(np.prod(delta_vecs[i, :] / t))) / delta_vecs[i, :]
#            eig_vecs[i, :] = permuted_v[i] / s
    ...
    def __extract_deflation_blocks(self, diag, deflate_precision):
        ...
        for i in range(diag.shape[0]):
            if block_repres == None or np.abs(diag[i] - block_repres) > deflate_precision:
                block_repres = diag[i]
    ...
    def __inv_deflate_eig_vecs(self, eig_vecs, givens_deflates, deflate_precision):
        for gd in reversed(givens_deflates):
            ...
            eig_vecs = self.__givens_transformer.transform(eig_vecs, gd[1], gd[2], givens_coeff)
        return eig_vecs
        ...
        for db in deflation_blocks:
            ...
            for i in reversed(range(1, m)):
                if np.abs(v_[db[i]]) < deflate_precision:
                    ...
                    givens_coeff = self.__givens_transformer.compute_coeffs(v_[db[i - 1]], v_[db[i]])
                    v_ = self.__givens_transformer.transform(v_.reshape(-1, 1), db[i - 1], db[i], givens_coeff)
                    nulled_poss.append(db[i])
    ...
    def decompose(self, diag, v, ro=1.0, sorted_eig_sys=False):
        ...
        cond_num = float(np.linalg.norm(permuted_diag) + permuted_v.reshape(1, -1).dot(permuted_v.reshape(-1, 1)))
        ...
        deflated_eig_vecs = self._eig_vecs(eig_val_res[1], nontrivial_v, nontrivial_diag)
        eig_vals = np.zeros(n)
        ...
        eig_vecs = self.__inv_deflate_eig_vecs(eig_vecs, deflation_res[3], deflate_precision)
        
        if sorted_eig_sys == False:
...
class async_rom_des_eigen_solver(rom_des_eigen_solver):
    ...
    def _eig_vals(self, v, diag):
        ...
        async_results = [processor_pool.apply_async(async_rom_eig_vals, (self._eig_solver, (i, v, diag))) for i in range(n)]  
    ...
    def _eig_vecs(self, delta_vecs, v, diag):
        ...
        processor_pool = mproc.Pool(self._num_threads)
        async_results = [processor_pool.apply_async(async_rom_eig_vec, (i, delta_vecs[i, :], v, diag)) for i in range(n)]
        processor_pool.close()
        processor_pool.join() 
        for i in range(n):
...
class quad_solver(object):
    ...
    def maximizer(self, eig_sys, b, r):
        ...
        if b != None:
            ...
            pinv_solution = eig_sys[1].dot((d.reshape(-1) * diag).reshape(-1, 1)).reshape(-1)
            pinv_sol_norm = np.linalg.norm(pinv_solution) 
            if np.abs(pinv_sol_norm - r) < self.__degenerate_sol_precision:
                warnings.warn('unique solution (using pseudo-inverse) computed successfully!')
...
class greedy_dir_solver(object):
    ...
    def _next_horth_direction(self, eig_sys, b, r, qr_sys, alpha):
        if None == qr_sys and None == alpha:
            return (self._next_direction(eig_sys, b if self.__nonzero_vec(b) else None, r)[0], None, eig_sys)
        
        if qr_sys == None:
#            Q, R = spla.qr(alpha.reshape(-1, 1))
#            qr_sys = (Q, R)
#            U = Q.T.dot(eig_sys[1])
            res = self.qr_solver.qr_transform(eig_sys[1], np.identity(alpha.shape[0]), None, alpha)
            qr_sys = (res[0], res[1])
            U = res[2]
        else:
...
#        U = quad_eig_sys[1].dot(rou_eig_sys[1]) # !!! avoid matrix multiplication
    ...
    def sph_cl_weighted_var_term_eig_sys(self, X, y, label_mask, kernel_sys, sh_paths_cache='sh_paths_cache.dat'):
...
#        sh_paths = utils.async_adj_mat_to_dijkstra_spath_mat(g)
        ...
        W = pos_sub_mat_kernel.T.dot(H_pos).dot(pos_cl_weigths).dot(H_pos).dot(pos_sub_mat_kernel)
        W += neg_sub_mat_kernel.T.dot(H_neg).dot(neg_cl_weigths).dot(H_neg).dot(neg_sub_mat_kernel)
...
#        W -= float(1.0 / m2) * true_neg_sub_mat_kernel.T.dot(H_true_neg).dot(true_neg_sub_mat_kernel)
    ...
    def wvar_cl_mode_directions(self, sph_quad_eig_sys, y, label_mask, kernel_sys, params):        
        ...
        return self.direction_solver.directions(kernel_sys, sph_quad_eig_sys, -0.5 * sph_kb_terms[1], params['r'], mu)
    
    def wvar_cl_mode_ho_directions(self, sph_quad_eig_sys, y, label_mask, kernel_sys, params):
        labels = self.__pack_cl_labels(y, label_mask, params)
        sph_kb_terms = self.__interpret_cl_constraint(kernel_sys, labels, label_mask, params)
    ...
    def cl_mode_ho_directions(self, sph_quad_eig_sys, y, label_mask, kernel_sys, params):
        labels = self.__pack_cl_labels(y, label_mask, params)
        sph_kb_terms = self.__interpret_cl_constraint(kernel_sys, labels, label_mask, params)
        return self.direction_solver.hard_orth_directions(kernel_sys, sph_quad_eig_sys, -0.5 * sph_kb_terms[1], params['r'], params['dim'])
    ...
    def soft_cp_mode_ho_directions(self, sph_quad_eig_sys, label_mask, y, kernel_sys, params, const_nu):
        lin_term = self.__interpret_cp_lin_constraint(kernel_sys, y, label_mask, const_nu)
        return self.direction_solver.hard_orth_directions(kernel_sys, sph_quad_eig_sys, 0.5 * lin_term, params['r'])
